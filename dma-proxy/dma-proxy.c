 ...
 *
 * Some code modifications and bug fixes: Copyright (C) 2024 IMSAR, LLC.
 *
...
static void start_transfer(struct dma_proxy_channel *pchannel_p)
{
	...
	dev_info(pchannel_p->proxy_char_dev_p, "start_transfer: buf_descr_index = %d, dma_handle = 0x%px, length = %u",
	         buf_descr_index, (void *)pchannel_p->buf_descr_table[buf_descr_index].dma_handle,
	         pchannel_p->buffer_table_p[buf_descr_index].length);
	...
	if (!chan_desc)
	{
		dev_err(pchannel_p->proxy_char_dev_p, "dmaengine_prep*() error\n");
		return;
	}
	...
	{
		dev_err(pchannel_p->proxy_char_dev_p, "Submit error\n");
		return;
	}
...
static void wait_for_transfer(struct dma_proxy_channel *pchannel_p)
{
	...
	if (timeout == 0)
	{
		...
		dev_err(pchannel_p->proxy_char_dev_p, "DMA timed out\n");
		return;
	}
	...
	if (status != DMA_COMPLETE)
	{
		...
		dev_err(pchannel_p->proxy_char_dev_p, "DMA returned completion callback status of: %s\n",
...
static long dma_proxy_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	dev_info(pchannel_p->proxy_char_dev_p, "dma_proxy_ioctl: buf_descr_index = %u, cmd = %u",
	         pchannel_p->buf_descr_index, cmd);

	// Perform the DMA transfer on the specified channel blocking til it completes
	switch (cmd)
	{
	case START_XFER:
		if (pchannel_p->buffer_table_p[pchannel_p->buf_descr_index].length <= 0)
		{
			dev_err(pchannel_p->proxy_char_dev_p, "invalid length for request");
			return -EINVAL;
		}
...
static int cdevice_init(struct dma_proxy_channel *pchannel_p, char *name)
{
	...
	pchannel_p->proxy_char_dev_p = device_create(s_class_p, NULL, pchannel_p->char_dev_node, NULL, name);
...
static void cdevice_exit(struct dma_proxy_channel *pchannel_p)
{
	...
	{
		device_destroy(s_class_p, pchannel_p->char_dev_node);
		cdev_del(&pchannel_p->cdev);
		unregister_chrdev_region(pchannel_p->char_dev_node, 1);
	}
}
...
static int create_channel(struct platform_device *pdev, struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
	...
	for (buffer_descr = 0; buffer_descr < BUFFER_COUNT; buffer_descr++)
	{
		...
		dev_info(pchannel_p->dma_dev_p, "buffer %u: busaddr=%px", buffer_descr,
		         (void *)pchannel_p->buf_descr_table[buffer_descr].dma_handle);
	}
...
static int dma_proxy_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "dma_proxy_probe: using %u buffers of size %u bytes\n", BUFFER_COUNT, BUFFER_SIZE);
...
static int dma_proxy_remove(struct platform_device *pdev)
{
	...
	dev_info(dev, "dma_proxy module exiting\n");
