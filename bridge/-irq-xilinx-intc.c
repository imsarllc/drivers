...
static void intc_handler(struct irq_desc *desc)
{
	...
	struct intc *local_intc = irq_data_get_irq_handler_data(&desc->irq_data);
	int irq;
...
static int __init xilinx_intc_of_init(struct device_node *node, struct device_node *parent)
{
	u32 irq;
	int ret;
	struct intc *intc;
	...
	intc->domain = irq_domain_add_linear(node, intc->nr_irq, &xintc_irq_domain_ops, intc);
