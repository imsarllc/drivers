...
// globals
struct struct device *logging_device;
...
static ssize_t intc_read(struct file *f, char __user *buf, size_t bytes, loff_t *off)
{
	...
	{
		timeout_jiffies = (file_data->timeout * HZ) / 1000;
	}
	...
	if (status == 0) // timeout
	{
		return (file->f_flags & O_NONBLOCK) ? -EAGAIN : -ETIME;
	}
	...
	{
		dev_err(device_data->device, "read called with size %d (need %d)\n", bytes, sizeof(count));
		return -EINVAL;
	}
...
static long intc_irq_init(int num)
{
	...
	for (ii = 0; ii < INTC_IRQ_COUNT; ii++)
	{
		fid[ii].interrupt_count = 0;
		spin_lock_init(&fid[ii].consumers_spinlock);
...
static int intc_open(struct inode *inode, struct file *f)
{
	...
	file_data->timeout = dev_data->default_timeout;
...
static int intc_close(struct inode *inode, struct file *f)
{
	...
	list_is_empty = imsar_user_interrupt_consumer_remove(dev_data, file_data);
	if (list_is_empty)
	{
		intc_enable(ii, 0);
	}
...
static int intc_of_remove(struct platform_device *of_dev)
{
	...
	class_destroy(cl);
	unregister_chrdev_region(dev, INTC_IRQ_COUNT);
	dev_info(logging_device, "exit: unregistered\n");
...
static int intc_of_probe(struct platform_device *ofdev)
{
	...
	for (ii = 0; ii < INTC_IRQ_COUNT; ii++)
	{
		...
		fid[ii].default_timeout = MAX_SCHEDULE_TIMEOUT; // disable timeouts
	}
